...
이제 **회귀(regression)** 의 의미를 알았으니 , 회귀분석이 무엇인지 , 회귀식이 무엇인지에 대해서 한번 알아보자. <br>
...
그래서 이번 보고서에서는 한 가지 원인이 한 가지의 결과에만 영향을 미치는 **단순회귀분석(simple regression)** 을 할 것이다.
...
**회귀분석**는 변수들 사이의 인과관계를 규명하고자 하는 분석방법이기 때문에 , 변수의 역할설정이 굉장히 중요하다. 


|연구 주제|
|------|
|학생의 수면 시간이 시험 점수에 영향을 미치는가?|
|카페인이 식욕에 어떠한 영향을 미치는가?|
